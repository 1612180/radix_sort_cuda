...
void sortByDevice(const uint32_t *in, int n, uint32_t *out, int nBits,
  ...
  // prepare for historgram kernel and scan kernel
  ...
  // loop
  // src -> d_src
  // histogram kernel on d_src -> d_hist
  // scan kernel on d_hist -> d_histScan
  // d_histScan -> histScan
  // scatter histScan with src -> dst
  // swap src <-> dst
  // end loop
