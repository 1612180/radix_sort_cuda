...
// 1)
__global__ void computeHist2DKernel43(uint32_t *in, int n, uint32_t *hist,
  ...
  for (int bin = threadIdx.x; bin < nBins; bin += blockDim.x) {
    atomicAdd(&hist[bin * gridDim.x + blockIdx.x], sHist[bin]);
  }
}

// in -> out, blockSums
__global__ void scanBlockKernel43(uint32_t *in, int n, uint32_t *out,
                                  uint32_t *blockSums) {
...
__global__ void addScannedBlockSums43(uint32_t *out, int n,
                                      uint32_t *blockSums) {
...
__global__ void inclusiveToExclusive43(uint32_t *in, int n, uint32_t *inclusive,
                                       uint32_t *exclusive) {
  int i = blockDim.x * blockIdx.x + threadIdx.x;
  if (i < n) {
...
__global__ void convertBinary43(uint32_t *in, int n, uint32_t *inBinary,
...
__global__ void calculateRankPerBlock43(uint32_t *inBinary, int n,
...
__global__ void coutingSortPerBlock43(uint32_t *in, int n,
...
// decrease by 1
// 0 1 0 1 0 1 0 1 2
__global__ void scanBlockKernelWithEqual43(uint32_t *in, int n,
...
__global__ void coutingSortWithHist43(uint32_t *in, int n, uint32_t *out,
...
void sortBase43(uint32_t *in, int n, uint32_t *out, int nBits,
                int *blockSizes) {
  ...
  for (int bitBig = 0; bitBig < sizeof(uint32_t) * 8; bitBig += nBits) {
    ...
    computeHist2DKernel43<<<histogramGridSize, histogramBlockSize,
                            nBins * sizeof(uint32_t)>>>(d_in, n, d_hist, nBins,
                                                        bitBig);
    CHECK(cudaDeviceSynchronize());

    // d_hist -> d_histScan
    scanBlockKernel43<<<scanHistogramGridSize, scanHistogramBlockSize,
    ...
    scanBlockKernelWithEqual43<<<otherGridSize, otherBlockSize,
                                 otherBlockSize * sizeof(uint32_t)>>>(
        d_in, n, d_outWithEqual, nBins, bitBig);
    CHECK(cudaDeviceSynchronize());

    coutingSortWithHist43<<<histogramGridSize, histogramBlockSize>>>(
